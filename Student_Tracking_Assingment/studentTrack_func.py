...
def first_run(self):
    ...
    with conn:
        ...
        if count < 1:
            cur.execute("""INSERT INTO tbl_studentTrack (col_fname,col_lname,col_fullname,col_phone,col_email,col_course) VALUES (?,?,?,?,?,?)""", ('John','Doe','John Doe','111-111-1111','jdoe@email.com','class'))
            conn.commit()
    conn.close()
...
def onSelect(self,event):
    ...
    with conn:
        ...
        # This returns a tuple and I can slice it into 4 parts using data[] during the iteration
        for data in varBody:
...
def addToList(self):
    ...
    if (len(var_fname) > 0) and (len(var_lname) > 0) and (len(var_phone) > 0) and(len(var_email) > 0) and (len(var_course) > 0): # enforce the user to provide both names
        ...
        with conn:
            cursor = conn.cursor()
            # Check the database for existance of the fullname, if so I will alert user and disregard request
            cursor.execute("""SELECT COUNT(col_fullname) FROM tbl_studentTrack WHERE col_fullname = '{}'""".format(var_fullname))#,(var_fullname))
            count = cursor.fetchone()[0]
            chkName = count
            if chkName == 0: # if this is 0 then there is no existance of the fullname and I can add new data
...
def onDelete(self):
    ...
    with conn:
        ...
        # the database...cannot delete last record or I will get an error
        cur.execute("""SELECT COUNT(*) FROM tbl_studentTrack""")
        count = cur.fetchone()[0]
        if count > 1:
